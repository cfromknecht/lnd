...
func testSendToRouteAMP(net *lntest.NetworkHarness, t *harnessTest) {
	...
	childPreimages := make(map[lntypes.Preimage]struct{})
	...
	for i, hops := range sendRoutes {
		...
		childPreimages[child.Preimage] = struct{}{}
	...
	// invoice.
	childPreimagesCopy := make(map[lntypes.Preimage]struct{})
	for preimage := range childPreimages {
		childPreimagesCopy[preimage] = struct{}{}
	}
	...
	for _, htlc := range rpcInvoice.Htlcs {
		...
		_, ok := childPreimages[childPreimage]
		require.True(t.t, ok)
